...
$(function(){
	...
	$("#update,#addSchedule").click(function(){
		 ...
		 if(checkedBoxs.length == 1){
			 ...
			 $('#myModal').modal() ;
			 // 加载一个排诊界面 
			 $("#paizhenxinxi").attr("src","${pageContext.request.contextPath}/ScheduleServlet?method=addSchedule&methodId="+this.id+"&id="+$(checkedBoxs).val());
		 }else{
...
<body style="width:100%;height:100%;">
    <!-- 工具按钮区 -->
		<form class="form-horizontal"
			action="${pageContext.request.contextPath }/ScheduleServlet?method=list"
			method="post" >
				...
				<tbody>
					<tr>
						...
						<td>
							...
							</button> <a   id="addSchedule" class="btn btn-success"><span
								class="glyphicon glyphicon-plus"></span>&nbsp;发布排诊信息</a>
						</td>
					</tr>
				</tbody>
			</table>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="myModalLabel">添加排诊信息</h4>
					</div>
					<div class="modal-body" >
						 <!-- 放置排诊内容的区域  -->
						 <iframe frameborder="0" width="100%" height="270px" id="paizhenxinxi" >
						    </iframe>
					</div>
	...
	<center>
		<div class="row" >
		   ...
		   <nav>
			  <ul class="pagination">
			    <li id="first">
			      <a href="${pageContext.request.contextPath }/ScheduleServlet?method=list&pageIndex=1&name=${doctor.name}&partCode=${doctor.part.partCode}">首页</a>
			    </li>
			    <li id="pre"><a href="${pageContext.request.contextPath }/ScheduleServlet?method=list&pageIndex=${pageModel.pageIndex - 1}&name=${doctor.name}&partCode=${doctor.part.partCode}">上一页</a></li>
			    <li id="next"><a href="${pageContext.request.contextPath }/ScheduleServlet?method=list&pageIndex=${pageModel.pageIndex+1}&name=${doctor.name}&partCode=${doctor.part.partCode}">下一页</a></li>
			    <li id="last">
			      <a href="${pageContext.request.contextPath }/ScheduleServlet?method=list&pageIndex=${pageModel.pageCount}&name=${doctor.name}&partCode=${doctor.part.partCode}">
			              尾页
			      </a>
			    </li>
