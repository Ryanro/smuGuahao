...
public class VerifyAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		for (int i = 0; i < 4; i++) {
			...
			code += temp;
			/** 设置画笔的颜色(颜色是随机生成) */
			g.setColor(new Color(random.nextInt(20), random.nextInt(40), random
					.nextInt(20)));
		...
		// 把当前请求客户端的验证码存入到它的Session中
		request.getSession().setAttribute(Constants.SESSION_CODE, code);
		/** 消毁画笔 */
		g.dispose();
		/** 输出 */
		try {
